...
- VMWare / VirtualBox 等虛擬機（建議記憶體 2GB 以上）
- 或 Windows WSL2（不建議，一些行為可能需額外調整）
...
- 快取過後的遊戲名稱不會自動隨 Steam 名稱異動，僅當每日 1:05 或你手動快取時才會補上新遊戲。

### 目前大部份皆可執行，部份未驗證

- 好友列表 / 持有遊戲要公開，不然api抓不到會變空值。  
- 持有遊戲如果太多，這應用程式可能不適合你，因為程式在系統時間1:05時、在背景試著執行遊戲名稱預先快取，一個遊戲約需要10~15秒，如果你持有的遊戲很多，可能會一天都跑不完、或者是因api使用太頻繁、會被Steam拒絕存取。一旦第一次執行過後，再來只會處理更新部份，除非資料被刪掉。  
